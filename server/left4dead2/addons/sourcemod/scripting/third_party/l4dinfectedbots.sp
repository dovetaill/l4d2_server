...
#define PLUGIN_VERSION             "3.0.8-pve.1-2026/9/22"
#define DEBUG 0
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("L4DInfectedBots_SetDynamicPolicy", Native_SetDynamicPolicy);
...
public int Native_SetDynamicPolicy(Handle plugin, int numParams)
{
	int maxSpecials = GetNativeCell(1);
	float spawnTimeMin = view_as<float>(GetNativeCell(2));
	float spawnTimeMax = view_as<float>(GetNativeCell(3));
	int weights[6];
	for (int index = 0; index < sizeof(weights); index++)
	{
		weights[index] = GetNativeCell(index + 4);
	}

	if (maxSpecials < 0 || maxSpecials > 30)
	{
		ThrowNativeError(SP_ERROR_PARAM, "maxSpecials must be between 0 and 30");
		return 0;
	}
	if (spawnTimeMin < 3.0 || spawnTimeMax < spawnTimeMin || spawnTimeMax > 180.0)
	{
		ThrowNativeError(SP_ERROR_PARAM, "spawn interval must satisfy 3 <= min <= max <= 180");
		return 0;
	}
	for (int index = 0; index < sizeof(weights); index++)
	{
		if (weights[index] < 0 || weights[index] > 1000)
		{
			ThrowNativeError(SP_ERROR_PARAM, "class weights must be between 0 and 1000");
			return 0;
		}
	}

	bool changed = g_ePluginSettings.m_iMaxSpecials != maxSpecials
		|| g_ePluginSettings.m_fSpawnTimeMin != spawnTimeMin
		|| g_ePluginSettings.m_fSpawnTimeMax != spawnTimeMax;
	for (int index = 0; index < sizeof(weights); index++)
	{
		changed = changed || g_ePluginSettings.m_iSpawnWeight[index] != weights[index];
	}
	if (!changed)
	{
		return 1;
	...
	for (int survivorCount = 0; survivorCount <= L4D_MAXPLAYERS; survivorCount++)
	{
		ePluginData[survivorCount].m_iMaxSpecials = maxSpecials;
		ePluginData[survivorCount].m_fSpawnTimeMin = spawnTimeMin;
		ePluginData[survivorCount].m_fSpawnTimeMax = spawnTimeMax;
		ePluginData[survivorCount].m_fCoopVersSpawnTimeMin = spawnTimeMin;
		ePluginData[survivorCount].m_fCoopVersSpawnTimeMax = spawnTimeMax;
		for (int index = 0; index < sizeof(weights); index++)
		{
			ePluginData[survivorCount].m_iSpawnWeight[index] = weights[index];
		}
	}

	g_ePluginSettings.m_iMaxSpecials = maxSpecials;
	g_ePluginSettings.m_fSpawnTimeMin = spawnTimeMin;
	g_ePluginSettings.m_fSpawnTimeMax = spawnTimeMax;
	g_ePluginSettings.m_fCoopVersSpawnTimeMin = spawnTimeMin;
	g_ePluginSettings.m_fCoopVersSpawnTimeMax = spawnTimeMax;
	for (int index = 0; index < sizeof(weights); index++)
	{
		g_ePluginSettings.m_iSpawnWeight[index] = weights[index];
	}

	z_max_player_zombies.SetInt(maxSpecials);
	CheckIfBotsNeeded2();
	return 1;
}
