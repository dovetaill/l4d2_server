...
{
    RegPluginLibrary("dual_primaries");
    CreateNative("L4D2DualPrimaries_HasAlternate", Native_HasAlternate);
    CreateNative("L4D2DualPrimaries_Switch", Native_Switch);
...
public any Native_HasAlternate(Handle plugin, int numParams)
{
    int client = GetNativeCell(1);
    return client > 0 && client <= MaxClients && IsClientInGame(client) && IsPlayerAlive(client)
        && !g_IsMapTransitionActive && g_PrimarySlot2[client].isValid
        && g_PrimarySlot2[client].classname[0] != '\0';
}

public any Native_Switch(Handle plugin, int numParams)
{
    int client = GetNativeCell(1);
    if (!Native_HasAlternate(plugin, 1))
    {
        return false;
    }
    return Cmd_SwitchPrimary(client, 0) == Plugin_Handled;
}
