#define PLUGIN_NAME "[L4D1/2] InfectedBots Spawn API"
#define PLUGIN_AUTHOR "Shadowysn, ProdigySim (Major Windows Fix), Harry"
#define PLUGIN_DESC "Native-only spawn implementation dependency for InfectedBots."
#define PLUGIN_VERSION "1.6h-pve.1-2026/9/22"
...
{
	char caller[PLATFORM_MAX_PATH];
	GetPluginFilename(plugin, caller, sizeof(caller));
	if (!StrEqual(caller, "l4dinfectedbots.smx", false))
	{
		ThrowNativeError(SP_ERROR_NOT_RUNNABLE, "NoLimit_CreateInfected is restricted to l4dinfectedbots.smx");
		return -1;
	}
...
public void OnPluginStart()
{
	...
	// No console command is registered: InfectedBots is the only allowed caller.
}
